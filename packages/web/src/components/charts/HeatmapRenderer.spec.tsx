...
describe("HeatmapRenderer", () => {
  ...
  describe("geometry integrity", () => {
    ...
    it("fills the measured container once the ResizeObserver reports a size", () => {
      // jsdom has no ResizeObserver and reports clientWidth/Height 0, so the
      // fallback size is all the other tests ever exercise. Stub both to prove the
      // grid actually grows to the widget instead of staying at FALLBACK_*.
      ...
      try {
        ...
        expect(Number(svg.getAttribute("height"))).toBeGreaterThan(500);
        // Cells share the extra room rather than leaving the plot area blank.
        const r = document.querySelector("rect")!;
        expect(Number(r.getAttribute("width"))).toBeGreaterThan(400);
      } finally {
