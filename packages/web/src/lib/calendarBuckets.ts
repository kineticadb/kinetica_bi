 ...
 * epoch (ms). Mirrors parseUTCMs from calendarLayout.ts.
 */
function parseKeyMs(key: string): number {
  const iso = key.includes("T") ? key : key.replace(" ", "T") + "Z";
...
export function enumerateGroupBuckets(
  ...
  if (subdomain === "hour") {
    switch (domain) {
      ...
      case "week": {
        // Snap to Monday of the week containing domainKey
        const dayTrunc = Date.UTC(y, mo, domainDay, 0, 0, 0, 0);
        const dow = new Date(dayTrunc).getUTCDay(); // 0=Sun … 6=Sat
        const offset = (dow + 6) % 7; // distance back to Monday (Mon=0)
        const weekStart = dayTrunc - offset * 86_400_000;
        // 7 days × 24 hours = 168 hour keys
  ...
  if (subdomain === "day") {
    switch (domain) {
      case "week": {
        // Snap to Monday of the week containing domainKey
        const dayTrunc = Date.UTC(y, mo, domainDay, 0, 0, 0, 0);
        const dow = new Date(dayTrunc).getUTCDay();
  ...
  if (subdomain === "week") {
    switch (domain) {
      case "month": {
        // Include Monday-anchored week-starts whose START falls within (y, mo)
        // Walk from the Monday on-or-before the 1st of the month
      ...
      case "year": {
        // Include Monday-anchored week-starts whose START falls within year y
        const yearStart = Date.UTC(y, 0, 1, 0, 0, 0, 0);
