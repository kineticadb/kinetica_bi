...
export const createApp = async (): Promise<express.Express> => {
  ...
  app.get("/api/branding/logo", (_req, res) => {
    ...
    if (!logoData || !logoMime) return res.status(404).end();
    const buf = Buffer.from(logoData, "base64");
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      // Phase 83 Plan 04 (BRANDUI-06): branch on variant field from multipart form.
