...
**Requirements**: CALUX-V113-03 (gap-fill portion DONE; live week-anchor verification carried to Phase 69)
**Depends on:** Phase 68.1
**Plans:** 3/3 plans complete (verified 2026-06-17)

Plans:
- [x] 68.2-01-PLAN.md — Pure `calendarBuckets.ts` per-group date-range bucket enumerator (TDD): `enumerateGroupBuckets` → in-range subdomain keys in "YYYY-MM-DD HH:mm:ss" UTC (29 tests, all 8 combos incl. leap)
- [x] 68.2-02-PLAN.md — Live Kinetica `DATE_TRUNC('week')` anchor probe — NOT-RUN (auth-gated, `.env` security-prohibited); `WEEK_START=1` retained; re-flagged for Phase 69
- [x] 68.2-03-PLAN.md — Rewired `gapFillCalendar` to per-group (consumes `enumerateGroupBuckets`): in-range grey, out-of-range blank, week×day single column; Phase-67 spec updated to per-group; renderer call + tests; invariants preserved
...
| 68.1. calendar-ux-wrapped-layout-on-widget-controls | 3/3 | Complete | 2026-06-17 |
| 68.2. calendar-week-anchor-spike-per-group-gap-fill | 3/3 | Complete | 2026-06-17 |
