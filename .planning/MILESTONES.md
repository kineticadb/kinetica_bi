...
**Phases completed:** 1 phase (118), 3 plans

**Delivered:** The layers panel now distinguishes drawing / zoom-inactive / eye-off layers, shows the zoom range that would bring a dimmed layer back, and updates live as the operator zooms — in both the in-map legend and the standalone Legend widget, degrading to today's appearance when the bound map's live zoom is unavailable. 7/7 ZLGND-V123 requirements; operator UAT 8/8 checks PASS with nothing recorded as not-exercised. Client-only throughout — `packages/server` untouched, no new dependency. Tag `v1.23.0` (three-part per RELEASING.md).

**Key accomplishments:**

- **One shared zoom-range predicate** (118-01) — `lib/zoomRangeBounds.ts`'s `toOlZoomBounds`/`isLayerActiveAtZoom` is now the SOLE source of truth for "is this layer drawing at this zoom," consumed by both `MapChartRenderer.tsx`'s `applyZoomRangeToLayer` (the OL-application path) and its info-click gate — confirmed by an independent grep audit that no third implementation exists anywhere in the tree. The boundary oracle (for `{minZoom:3, maxZoom:10}`: active at 2.9, 3, 10; inactive at 10.5) was derived independently three times — research, plan checker, verifier — all agreeing.
- **A pre-existing SHIPPED bug found and fixed, not deferred** (118-01) — the info-click gate (`isLayerVisibleAtCurrentZoom`) had re-implemented the zoom-range check with raw inclusive bounds, diverging from what OpenLayers actually draws: at a fractional zoom just below a layer's `minZoom`, the layer was visibly on the map but info-clicks on it were silently swallowed. Found by phase research, put in scope by explicit operator decision — overriding the researcher's own "log as tech debt" recommendation — and fixed, with two named regression tests (`P5z4`/`P5z5`) plus its own operator UAT check.
- **Three-state legend rendering** (118-02) — `resolveLegendLayers` gained an optional live-zoom parameter and a three-state `zoomActive` contract (`true`/`false`/`undefined`, never coerced); `LayersLegendPanel` renders a distinct dimmed treatment (three numerically-distinct opacities + a `var(--warning)` left-rule, mutually exclusive from eye-off `hidden` styling by construction) plus a configured-range chip (`zoom 3–10`) on zoom-limited rows. A layer with no configured range renders exactly as it did before (exact-className regression-locked, not mere non-crash).
- **Live wiring to both consumers** (118-03) — `MapChartRenderer.tsx`'s in-map legend and `LegendRenderer.tsx`'s standalone Legend widget each gained a reactive, widget-scoped selector into `mapCurrentViewStore`'s live fractional zoom (mirroring the `MapConfigPanel.tsx` scoped-selector precedent), so the panel updates live as the operator zooms with no reload or re-open. The standalone widget's unknown-zoom case degrades through the exact same path as "no range configured" (never the orphan UI) — verified by threading the value through with zero coercion (`?? 0`/`?? false` both absent, confirmed structurally and by mutation probe).
- **Operator UAT closing all 7 requirements** (118-03) — 8/8 PASS covering the regression canary, active/zoom-inactive/eye-off distinctness in BOTH themes (separately, not assumed from one), live zoom tracking, the standalone widget's mirror + fallback behaviour, the deliberate info-click fix, and the operator's original complaint. Unlike Phases 115-117, every check was runnable and every one was run — nothing recorded as not-exercised.
- **The web vitest suite's cross-run non-determinism was root-caused and fixed this milestone** — carried from v1.22 as `TD-V122-TEST-FLAKE`, this was NOT a v1.23 requirement but shipped alongside it and directly enabled it: every Phase 118 verification passed on the first attempt with no re-runs, the first time that has been true in this project's recent history. See Known Gaps below for the full finding.

**Known gaps and standing costs — recorded here deliberately, not smoothed over:**

- **The web vitest suite's non-determinism (carried from v1.22 as `TD-V122-TEST-FLAKE`) was root-caused to TWO distinct one-line async-query defects, not "cross-mode contamination" as first assumed.** (1) Testing Library's `findBy*`/`waitFor` default `asyncUtilTimeout` (1000ms) was never configured, while vitest's own `testTimeout` sits at 5000ms — at full-suite scale, jsdom setup dominates wall-clock and a sub-second render can exceed the 1s window, reporting "Unable to find…" which reads exactly like an assertion failure (this is why it was misdiagnosed twice: first as CPU oversubscription from concurrent orchestrator agents — refuted by reproducing with a single vitest instance on an idle machine — then as deterministic contamination — refuted by bisection, which proved no contaminating file exists). (2) A synchronous `getBy*` in `App.tableDeeplink.spec.tsx` asserted on content that only exists after a LATER React commit than the one an immediately-preceding `await findBy*` had waited for — `asyncUtilTimeout` cannot help a query that never awaits at all. Fixed in `cde63ae` (9 lines, 2 files: `configure({ asyncUtilTimeout: 5000 })` in `test/setup.ts` + the one sync-after-async site). Verified: 0 of 4 clean full-suite runs before the fix, 4 of 4 after — including a near-miss where a single clean run after fixing defect 1 alone nearly ended the investigation before two confirmation runs immediately reddened on a different file, exposing defect 2. Full investigation trail — including four wrong diagnoses — preserved in `.planning/v123-flake-investigation-notes.md`. ~105 other "sync `getBy*` after `await findBy*`" sites remain across 12 spec files; most are safe, fix opportunistically per the carried tech-debt list, not as a sweep. **This casts doubt on the server's `TD-V16-TEST-ISOLATION` set-based gate, which carries the identical "cross-mode contamination" attribution and has never been tested this way** — worth re-examining before continuing to trust it.
- **theme-guard's `global.css` exemption was confirmed structurally, not just suspected.** `theme-guard.spec.ts` allowlists `global.css` and, for allowlisted files, asserts `hasHex === true` — it NEVER checks absence, so any hardcoded colour there passes every automated gate unconditionally. This milestone compensated with a hand-run hex+rgba audit (run three times independently: executor, orchestrator, verifier — all 0) plus SEPARATE light and dark operator checks (this is the exact hole that shipped Phase 114's `.onboarding-banner` light-mode defect). Narrowing the exemption to `:root` blocks remains unscheduled and is now a well-evidenced candidate.
- **~7 self-falsifying acceptance criteria occurred in this phase alone** (3 caught before execution, 4 during) — running total across Phases 115-118 is roughly 28-32. The dominant cause is a plan mandating a code comment containing the very token its own grep counts, inflating or deflating the expected number. None were satisfied by editing correct code to chase a broken count; in every case the real requirement was verified directly. Recorded as a planner-side defect worth fixing at the source (see CLAUDE.md's "Writing verifiable acceptance criteria" section), not executor error.
- **The OIDC path has STILL never been browser-verified** — four milestones running. `AUTH_MODE=password` on this instance, so the `kbi_returnTo` carry across a real IdP redirect remains covered by automated tests and mutation probes only.
- **26 mutation probes across the phase, zero non-firing** — the positive counterpart to the toothless-criteria finding above: where criteria were designed to fail on demand, they did, every time.

**Open tech debt carried to the backlog:** the theme-guard `global.css` exemption (above); `TLINK-F4` (three parallel dashboard/table URL implementations, reopening condition already recorded); `TLINK-F3` (sidebar Datasets/Dashboards no-ops while that record is open); `ZLGND-F1` (click a zoom-limited row to zoom the map into range), `ZLGND-F2` (live zoom in the panel header — considered, per-row ranges preferred instead), `ZLGND-F3` (warn at config time when a zoom range can't be satisfied by the map's own min/max zoom); ~105 remaining sync-after-async spec sites (fix opportunistically, not a sweep); the server's `TD-V16-TEST-ISOLATION` "cross-mode contamination" attribution (now suspect, given the web suite's equivalent turned out to be two one-line defects — worth re-testing the same way before continuing to trust the set-based gate); OIDC still never browser-verified, four milestones running.
...
| TD-02 | ~~Medium~~ → **Resolved for this repo (verified 2026-09-14)** | ORIGINAL ENTRY: "`kinetica_bi/server/.env` was tracked in git history (real Kinetica admin credentials in past commits). Untracked going forward via `git rm --cached`. **Rotate `KINETICA_PASSWORD` before any public push of this repository.**" <br><br>**AUDIT 2026-09-14 — the premise does not hold for THIS repository.** Checked before the v1.21 push and found no exposure: (1) `git log --all -- "*.env"` returns **0 commits** across all **823** commits — no `.env` file has ever been tracked here; (2) the only three commits referencing `KINETICA_PASSWORD` outside docs/examples (`9d99403`, `8aa4ad0`, `84d9738`) contain **placeholders only** — `replace-with-service-account-password`, `replace-me-with-a-long-random-string` — in `deploy/k3s.yaml` and `.env.example`; (3) `.gitignore:8` is `.env*` with `!.env.example`, and `packages/server/.env` is untracked today. <br><br>**Why the entry existed:** it cites the path `kinetica_bi/server/.env`, but this monorepo has always used `packages/server/`. The entry almost certainly describes a **predecessor repo or a pre-monorepo layout** and was carried forward into this file without being re-verified against the current history. <br><br>**Consequence:** pushing this repository does NOT expose credentials, and rotation is **not** a precondition for opening a PR. <br><br>**Still open elsewhere:** if those credentials were genuinely committed in some other repo, fork, or backup, that exposure is real and is unaffected by anything in this repository — rotation would still be warranted, just not on account of this git history. Only the operator can confirm whether that happened. Do not re-raise this as a blocker on this repo without re-running the three checks above. |
