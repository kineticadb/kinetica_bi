...
**Plans**: 1 plan
- [ ] 78-01-PLAN.md — useViewKeepAlive hook (per-view timers + AbortControllers, W-based re-arm, READ touch, teardown) + spec + static-import guard + mount in DashboardOpen
...
**Success Criteria** (what must be TRUE):
  ...
  3. The live walk-through attests PASS for the env-driven TTL flow (revised 2026-06-22 — Phase 74 pivoted from a runtime app-settings store/permission to env vars; there is NO settings UI or `app:manage_settings` permission to test): with `DEFAULT_VIEW_TTL_MINUTES` set to a non-default value (e.g. 10), newly materialized filter-views AND dynamic-views use the configured TTL (not the old hardcoded 5) and the client's expiry tracking reflects it; `TTL_KEEPALIVE_LEAD_MINUTES` is exposed on `GET /api/me` and consumed by the Phase 78 keep-alive.
